...
class RoleListView extends StatefulWidget {
  ...
  const RoleListView({Key? key, required this.onRoleSelected}) : super(key: key);
    ...
    return FutureBuilder<List<RoleModel>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          roles = snapshot.data!; // Assign the roles list from the snapshot data
          return Center(
            child: DropdownButton<String>(
              value: selectedRole?.id,
              onChanged: (String? newValue) {
                setState(() {
                  selectedRole = roles.firstWhere((role) => role.id == newValue);
                });
                widget.onRoleSelected(selectedRole); // Notify the parent widget about the selected role
              },
